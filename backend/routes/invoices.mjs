...
router.post("/invoices/generate-pdf", ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const checkPageBreak = (currentY, neededHeight = 20) => {
      if (currentY > doc.page.height - doc.page.margins.bottom - neededHeight) {
        doc.addPage();
    ...
    for (const section of data.sections || []) {
      ...
      doc.text(`Amount (${data.currency || "N/A"})`, LEFT_X + 340, tableTopY + 5, {
        width: 70,
        align: "right",
      });
      doc.text(
      ...
      for (const item of section.items || []) {
        ...
        doc.text(item.Product_name || "", LEFT_X + 5, rowY + 5, { width: 120 });
